...
if st.session_state.summary_output:
    ...
    # Use form for question input and submit button to avoid st.session_state modification issues
    ...
    if not st.session_state.show_study_plan:
        if st.button("🗓️ Create Study Plan"):
    ...
    if st.session_state.show_study_plan:
        ...
        plan_type = st.selectbox("📚 What kind of plan do you prefer?",
                                  ["Balanced", "Revision-Heavy", "Practice-Heavy"])

        if st.button("📘 Generate Study Plan", key="generate_study_plan"):
            with st.spinner("⚙️ Generating your study plan..."):
                try:
                    study_plan_prompt = """
